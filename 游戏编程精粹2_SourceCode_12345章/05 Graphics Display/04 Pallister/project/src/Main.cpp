...
{  
	...
	pApp = new ProceduralCloudDemo();

	//Our Window class contains a wrapper for the D3D device
	pApp->InitWorld( wnd.GetWrapper() );

	//Make it so the window in turn knows about the app and will keep it up to speed
	wnd.SetApp( pApp );

	//OK to start rendering
	wnd.Enable( true ); 
