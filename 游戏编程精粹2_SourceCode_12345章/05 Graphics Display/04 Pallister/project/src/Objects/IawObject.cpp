...
// have to convert this to be a derivative of object still. Maybe for Beta2
...
//lock the already-existing vb and write verts to it (used for dynamic objects)
HRESULT IawObject::UpdateVB()
{
  HRESULT hr;
  ...
  if (!mSharedVb)
  {
    if (mpIndexedVertexBuffer != NULL)
	{
		...
		DWORD dwLockFlags = D3DLOCK_NOSYSLOCK | D3DLOCK_DISCARD;
...
HRESULT IawObject::SetupIndexedVertexBuffer()
{
  ...
  if (!mSharedVb)
  {
    ...
    if (mpWrapper)
    {
      ...
      {
        //DWORD tempflags = 0;
        DWORD tempflags = D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC;
