...
void CRenderWnd::SetDefaultError(LPCTSTR lpErr)
{
    CNVEffectsDoc *pDoc = GetDocument();
    if (pDoc)
        pDoc->SetDefaultEffectError(lpErr);
}
...
void CRenderWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	CPoint point;
	GetCurrentPositionEx(GetDC()->GetSafeHdc(), (LPPOINT) &point);
	ScreenToClient(&point);

	pDoc->Keyboard(nChar, nFlags, true, point.x, point.y);
}
...
void CRenderWnd::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	pDoc->Keyboard(nChar, nFlags, false, point.x, point.y);
