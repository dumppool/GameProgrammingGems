...
CameraInterfaceHS::CameraInterfaceHS(bool wOn, int nCam)
{
	numCam=nCam;
	image=NULL;
	HSVimage=NULL;
...
CameraInterfaceHS::~CameraInterfaceHS()
{
...
//VISION
bool CameraInterfaceHS::calcHist()
{
	if(!capture)
		return false;
	...
	IplImage* planes[] = { h_plane, s_plane }; //the only needed plane is the hue

	int h_bins = 32; //number of bins in the histogram for the hue
	int s_bins = 32; //number of bins in the histogram for the saturation
	int hist_size[] = {h_bins, s_bins};
	float h_ranges[] = { 0, 180 }; // the hue is an angle, varies from 0 to 180 (360/2 to fit in 8 bits)
	float s_ranges[] = { 0, 255 }; // saturation varies from 0 (white, black, gray) to 255 (pure color)
    float* ranges[] = { h_ranges, s_ranges };

	if(hist!=0) cvReleaseHist( &hist ); //if already allocated, the histogram is released
	hist = cvCreateHist( 2, hist_size, CV_HIST_ARRAY, ranges, 1 ); //the histogram is created
...
int CameraInterfaceHS::queryState() //used to see the current position of the player's head
{
	clock_t start,end;
...
void CameraInterfaceHS::setWin(int state) //0: no win / 1: win / Other: switch
{
	switch(state)
	{
	case 0:
...
void CameraInterfaceHS::setDebug(bool state)
{
	debug_flag=state;
}