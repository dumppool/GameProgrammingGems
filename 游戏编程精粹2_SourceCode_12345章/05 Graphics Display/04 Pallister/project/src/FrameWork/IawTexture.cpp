...
const int IawTexture::BOTTOM_COLOR             = 0xFFFF0000; // blue
                             ...
                             BYTE* pBitsAlpha)
{
  ...
  //if this was forced in vidmem, or is a rendertarget, then we can't do this
  if ((mCreateFlags & IAW_TEXTR_FORCE_IN_VID_MEM) || (mCreateFlags & IAW_TEXTR_RENDER_TARGET))
	return E_FAIL;
...
HRESULT IawTexture::WriteFontToTexture()
{
  ...
  DeleteDC(alpha_bitmap); //bitmap->ReleaseDC(bitmap);
