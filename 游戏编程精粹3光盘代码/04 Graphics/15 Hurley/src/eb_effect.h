...
public:
	...
	{	
		...
		/*AddProperty(new EBProperty("EffectCaps", OBJECT_MEMBER(m_reqCaps), 
			EBTYPE_D3DCAPS_PROP, PROPFLAG_PRIVATE));*/
...
public:
    ...
    bool GetCurrentCaps(D3DCAPS8 *caps)
    {
        IDirect3D8 *g_pD3D;
    	HKEY hSectionKey;
    	long Err;
        ...
        RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\NVIDIA Corporation\\NVIDIA Effects Browser\\Settings", 0, "", REG_OPTION_NON_VOLATILE, KEY_READ, NULL, &hSectionKey, &disposition);
    
        if(hSectionKey) 
        {
    		RegQueryValueEx(hSectionKey, "Direct3D Caps", 0, &Type, NULL, &Size);
            Err = RegQueryValueEx(hSectionKey, "Direct3D Caps", 0, &Type, (LPBYTE)caps, &Size);
    		RegCloseKey(hSectionKey);
        }
        
        if (!hSectionKey || (Err != ERROR_SUCCESS))
        {
            if (NULL == (g_pD3D = Direct3DCreate8(D3D_SDK_VERSION)))
                return false;
        
            if (g_pD3D->GetDeviceCaps(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, caps) != D3D_OK)
                return false;
        
            if( g_pD3D != NULL)
                g_pD3D->Release();

            RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\NVIDIA Corporation\\NVIDIA Effects Browser\\Settings", 0, "", REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hSectionKey, &disposition);

        	if(hSectionKey)
	        {
		        Err = RegSetValueEx(hSectionKey, "Direct3D Caps", 0, REG_BINARY, (LPBYTE)caps, sizeof(D3DCAPS8));
		        RegCloseKey(hSectionKey);
