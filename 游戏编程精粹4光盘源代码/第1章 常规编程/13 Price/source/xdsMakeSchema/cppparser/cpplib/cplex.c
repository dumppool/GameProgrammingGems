...
#include <errno.h>
#include <fcntl.h>
#include "tcl.h"									// MTP (changed to local include)
...
//extern Tcl_Encoding encoding;`					//- MTP
static int yyeof;
...
int yylineno;
int yycharno;
//static Tcl_DString yybuf;							//- MTP
static unsigned char *yybuf;						//+ MTP
static unsigned long  yybuflen;						//+ MTP
static unsigned char *yyptr;

static Token_t TokenUnput;

//extern int f_ReadFile( int fd );					//- MTP
extern int f_ReadFile(FILE *fp);					//+ MTP
...
extern int f_ReadFile(FILE *fp)									//+ MTP (switch to POSIX I/O)
{
	// read entire file into yybuf
	...
	{
		yybuf = realloc(yybuf, yybuflen + nbytes);
		memcpy(yybuf+yybuflen, &buf[0], nbytes);
		yybuflen += nbytes;
	}
	if (nbytes < 0)
	{
...
#if 0
extern int
f_ReadFile (int fd)										//- MTP (switch to POSIX I/O)
{
