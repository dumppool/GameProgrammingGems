...
//+ MTP New function to one-by-one macro definition function (for command-line)
int AddMacro(int typ, char *macrodef)
{
	char *acBuffer;
	SearchEntry entry;
	char *sp;
	...
	int len = strlen(macrodef);
	if(typ == PAF_WORD_DEFINE)
		len += 7 + 1;		// length of "define "
	else if(typ == PAF_WORD_UNDEF)
		len += 6 + 1;       // length of "undef "
	else
		return 0;
    
	if(!(acBuffer = (char*)ckalloc(len)))
		return 0;

	if(typ == PAF_WORD_DEFINE)
	{
		sprintf(acBuffer, "define %s", macrodef);

		// change '#define ABC=DEF' to '#define ABC DEF'
		sp = strchr(acBuffer, '=');
		if(sp)
			*sp = ' ';
	}
	else
		sprintf(acBuffer, "undef %s", macrodef);
