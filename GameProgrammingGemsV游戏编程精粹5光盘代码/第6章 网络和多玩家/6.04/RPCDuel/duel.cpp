...
// every instance of that game.   {0F052F5A-79B4-4088-8F1E-B5DA9E67C2A4}
GUID g_AppGUID = { 0xf052f5a, 0x79b4, 0x4088, { 0x8f, 0x1e, 0xb5, 0xda, 0x9e, 0x67, 0xc2, 0xa4 } };
...
TCHAR     g_strAppName[256] = "RPCDuel";               // The name of the sample
    ...
    while( TRUE )
    {
        if( g_bIsActive )
        {
            ...
            else
            {
				...
				Sleep(1); /* none before single machine test */
...
LRESULT CALLBACK MainWndproc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( msg )
    {
        ...
        case WM_ACTIVATE:
            ...
            {
                // Deactivated
                g_bIsActive = TRUE/*before single machine test FALSE*/;
...
VOID ShowError( int iStrID )
{
    ...
    MessageBox( g_hwndMain, strMsg, TEXT("RPCDuel Message"), MB_OK );
}
...
VOID UpdateTitle()
{
    ...
    TCHAR strTitle[MAX_WINDOWTITLE] = TEXT("RPCDuel");
