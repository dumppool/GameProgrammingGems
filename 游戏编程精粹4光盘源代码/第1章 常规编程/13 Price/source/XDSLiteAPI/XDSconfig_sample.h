...
{
	extern void myProcessNode(unsigned short nodeType, void *nodeData, int nodeSize);
	myProcessNode(nodeType, nodeData, nodeSize);
}
...
//
inline void XDS_ERROR(const char *errText)
{
	fprintf(stderr, "%s\n", errText);
	exit(1);
}
