...
static SHIPTYPES g_ShipTypesUsed;
...
VOID LaunchGame()
{
    ...
    ZeroMemory( &g_ShipTypesUsed.m_types, sizeof(g_ShipTypesUsed.m_types) );
...
HRESULT InitOurShip()
{
    ...
    g_ShipTypesUsed.m_types[ship.byType] = 1;   // our color
...
/*before RPC
		...
		DuelProxy::RPC_Control(DPID_ALLPLAYERS,0,(BYTE)g_dwKeys);
    }
...
/*before RPC
	...
	DuelProxy::RPC_Sync(DPID_ALLPLAYERS,0,pShip->byType,pShip->cFrame,pShip->dPosX + pShip->dVelX*1000,pShip->dPosY + pShip->dVelY*1000);
}
...
/*before RPC        
            ...
            DuelProxy::RPC_BlockHit(DPID_ALLPLAYERS,0,row,col,mask);
...
/*before RPC
            ...
            DuelProxy::RPC_AddBlock(DPID_ALLPLAYERS,0,x,y);
        }
    }
}
...
/*before RPC
        ...
        DuelProxy::RPC_ShipHit(dpId,0,dpId);
    }
...
HRESULT ReceiveMessages()
{
    ...
    while( TRUE )
    {
        ...
        else if( SUCCEEDED(hr) /*before RPC&&
...
/*before RPC
                ...
                for( i = 0; i < NUM_SHIP_TYPES; i++ )
                    g_HostMsg.usedShipTypes[i] = g_ShipTypesUsed.m_types[i];

                SendGameMessage( (GENERICMSG*)&g_HostMsg, pAddMsg->dpId );                */
                DuelProxy::RPC_Host(pAddMsg->dpId,DPSEND_GUARANTEED,g_Blocks,g_ShipTypesUsed);
            }

            // Loop through the used ship types array and find the one that's been used the least
            for( i = 0; i < NUM_SHIP_TYPES; i++ )
            {
                if( (iLeastAmt < 0) || (g_ShipTypesUsed.m_types[i] < iLeastAmt) )
                {
                    iLeastAmt = g_ShipTypesUsed.m_types[i];
                    iLeastType = i;
                }
            }
            g_ShipTypesUsed.m_types[iLeastType]++; // update our copy of the used ship types
            break;
        }
        ...
        case DPSYS_DESTROYPLAYERORGROUP:
        {
            ...
            g_ShipTypesUsed.m_types[pShip->byType]--; // decrement the leaving players' ship type usage counter
...
VOID DoApplicationMessage( void* pMsg, DWORD dwMsgSize, DPID idFrom,
                           DPID idTo )
{
	CMessage msg((unsigned char*)pMsg);

	DuelStub::RPC_DoStub(idFrom,0,msg);
}

RPCResult DuelStub::RPC_Host(RPCSendTo recvFrom,RPCSendContext sendContext,const BLOCKS &Blocks,const SHIPTYPES &usedShipTypes)
{
	HRESULT hr;
	SHIP     ship;
	DWORD    dwSize = sizeof (ship);
	...
	if( !g_bHostPlayer )
	{
		...
		if( SUCCEEDED(hr)/* && dwMsgSize >= sizeof(HOSTMSG) */)
		{
			...
			{
				g_ShipTypesUsed.m_types[i] = usedShipTypes.m_types[i];
				if( (leastUsed < 0) || (g_ShipTypesUsed.m_types[i] < leastUsed) )
				{
					leastUsed   = g_ShipTypesUsed.m_types[i];
					ship.byType = (BYTE)i;
				}
			}
			g_ShipTypesUsed.m_types[ship.byType]++; // increment it because we used one
...
RPCResult DuelStub::RPC_BlockHit(RPCSendTo recvFrom,RPCSendContext sendContext,const BYTE &byRow,const BYTE &byCol,const BYTE &byMask)
{
	g_Blocks.bits[byRow][byCol] &= ~byMask;
	return RPCResult_Ok;
}

RPCResult DuelStub::RPC_AddBlock(RPCSendTo recvFrom,RPCSendContext sendContext,const BYTE &byX,const BYTE &byY)
{
	setBlock( byX, byY );
	return RPCResult_Ok;
}

RPCResult DuelStub::RPC_ShipHit(RPCSendTo recvFrom,RPCSendContext sendContext,const DPID &Id)
{
	HRESULT hr;
	SHIP        ship;
	DWORD       dwSize;
	...
	hr = DPUtil_GetPlayerLocalData( Id, &ship, &dwSize );
	if( FAILED(hr) )
		goto out;
	...
	if( !ship.bIgnore ) 
	{
		...
		if( Id == g_LocalPlayerDPID )
		{
			...
			DuelProxy::RPC_ShipHit(DPID_ALLPLAYERS,0,g_LocalPlayerDPID);
		}
	}
	// update player local data
	DPUtil_SetPlayerLocalData( Id, &ship, sizeof(ship) );
out:
	return RPCResult_Ok;
}

RPCResult DuelStub::RPC_Control(RPCSendTo idFrom,RPCSendContext sendContext,const BYTE &byState)
{
	SHIP ship;
	DWORD dwSize;
	...
	dwSize = sizeof(SHIP);
	// Get player local data
	HRESULT hr = DPUtil_GetPlayerLocalData( idFrom, &ship, &dwSize );
	if( FAILED(hr) )
		goto out;
	...
	// Update its State
	UpdateState( &ship, (DWORD)byState );
...
RPCResult DuelStub::RPC_Sync(RPCSendTo idFrom,RPCSendContext sendContext,const BYTE &byShipType,const char &cFrame,const double &dPosX,const double &dPosY)
{
	HRESULT hr;
    SHIP     ship;
    DWORD    dwSize;
    ...
    if( ship.bEnable )
    {
        ...
        ship.dVelX  = (dPosX - ship.dPosX)/1000;
        ship.dVelY  = (dPosY - ship.dPosY)/1000;
        ship.cFrame = cFrame;
    }
...
//                    for( int i = 0; i < NUM_SHIP_TYPES; i++ )
//                    {
//                        g_ShipTypesUsed.m_types[i] = pHostMsg->usedShipTypes[i];
//                        if( (leastUsed < 0) || (g_ShipTypesUsed.m_types[i] < leastUsed) )
//                        {
//                            leastUsed   = g_ShipTypesUsed.m_types[i];
//                            ship.byType = (BYTE)i;
...
RPCResult RPC_Send(RPCSendTo sendTo,RPCSendContext sendContext,CMessage &m)
{
    ...
    // Send the message to the relevant player(s)
    DPUtil_Send( g_LocalPlayerDPID, sendTo, dwFlags, m.GetBuffer(), m.GetCursor() );
...
void Message_Write(CMessage&m,const SHIPTYPES &val)
{
	m.Write(&val.m_types,sizeof(val.m_types));
}
void Message_Read(CMessage&m,SHIPTYPES &val)
{
	m.Read(&val.m_types,sizeof(val.m_types));
}

void Message_Write(CMessage&m,const BLOCKS &val)
{
	m.Write(&val.bits,sizeof(val.bits));
}
void Message_Read(CMessage&m,BLOCKS &val)
{
	m.Read(&val.bits,sizeof(val.bits));
}