...
bool IawWindow3d::Init(HINSTANCE hInst, int nCmdShow)
{
  ...
  //mWindow.hWnd = CreateWindow("Dx8Shell", "Dx8Shell", WS_OVERLAPPEDWINDOW,
  mWindow.hWnd = CreateWindow("Dx8Shell", "Intel D3D Appwizard Framework", WS_OVERLAPPEDWINDOW,
  ...
  {
    char szMsg[100];
    sprintf(szMsg, "Device failed to initialize.  Error: %8lx", hr);
    MessageBox(mWindow.hWnd, szMsg, "Initialize", MB_ICONEXCLAMATION | MB_OK);
...
bool IawWindow3d::BuildDeviceMenu()
{
    ...
    false))
  {
	...
	/* GRAYED THIS OUT DUE TO A BUG IN THE FRAMEWORK WITH PURE DEVICES
...
bool IawWindow3d::ProcessMessages()
{
  ...
  if (mpWrapper)
  {
    ...
    mpWrapper->SetFrameTime((float)(mCurTime - mLastTime));
  }
