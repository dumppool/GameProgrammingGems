...
//-----------------------------------------------------------------------------
D3D8Font_t::D3D8Font_t( TCHAR* strFontName, DWORD dwHeight, DWORD dwFlags )
{
...
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::InitDeviceObjects( LPDIRECT3DDEVICE8 pd3dDevice )
{
    HRESULT hr;
    ...
    DWORD dwBold   = (m_dwFontFlags&D3D8FONT_BOLD)   ? FW_BOLD : FW_NORMAL;
    DWORD dwItalic = (m_dwFontFlags&D3D8FONT_ITALIC) ? TRUE    : FALSE;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::RestoreDeviceObjects()
{
    HRESULT hr;
    ...
    for( UINT which=0; which<2; which++ )
    {
        ...
        if ( D3D8FONT_ZENABLE & m_dwFontFlags )
...
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::InvalidateDeviceObjects()
{
	if( m_pVB )
	{
		m_pVB->Release();
...
// Desc: Destroys all device-dependent objects
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::DeleteDeviceObjects()
{
	if( m_pTexture )
	{
		m_pTexture->Release();
...
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::GetTextExtent( TCHAR* strText, SIZE* pSize )
{
...
// Desc: Draws 2D text
//-----------------------------------------------------------------------------
HRESULT D3D8Font_t::DrawText( FLOAT sx, FLOAT sy, DWORD dwColor,
...
HRESULT D3D8Font_t::DrawBox( FLOAT fXPos, FLOAT fYPos, FLOAT fXSize, FLOAT fYSize, DWORD dwColor )
{
    if( m_pd3dDevice == NULL )
        return E_FAIL;
    ...
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0-0.5f,fYPos+fYSize-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0-0.5f,fYPos+0-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fXSize-0.5f,fYPos+fYSize-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fXSize-0.5f,fYPos+0-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fXSize-0.5f,fYPos+fYSize-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    *pVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0-0.5f,fYPos+0-0.5f,0.9f,1.0f), dwColor, 0.0f, 0.0f );
    dwNumTriangles += 2;
