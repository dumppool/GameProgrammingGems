                                   ...
                                   DWORD dwCoopLevel )
{
	...
	if( FAILED( hr = EAXDirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
        return DXTRACE_ERR( TEXT("DirectSoundCreate8"), hr );
	...
	void )
{
	...
	// DirectSound needs to be initialized before we can do anything
	if ( m_pDS == NULL )
	{
		MessageBox( NULL, "You must call CSoundManager::Initialize() before calling CSoundManager::InitializeEffects()", "Error", MB_OK );
		return S_FALSE;
	}

	m_pEAXListener = new CListenerEffectEAX();

	if ( FAILED( hr = m_pEAXListener->Initialize( m_pDS, &m_bEAXSupported )) )
		return DXTRACE_ERR( TEXT("m_pEAXListener->Initialize"), hr );
	
	// Set default envinroment
	if ( m_bEAXSupported )
	{
		m_pEAXListener->SetEnvironment( 1 );
...
//-----------------------------------------------------------------------------
// Name: CSoundManager::SetEnvironment()
// Desc: Sets EAX environment properties if supported.
//-----------------------------------------------------------------------------
HRESULT CSoundManager::SetEnvironment( 
	void )
{
	HRESULT hr;
	...
	{
		if ( FAILED( hr = m_pEAXListener->SetEnvironment()) )
			return DXTRACE_ERR( TEXT("m_pEAXListener->SetEnvironment"), hr );
	}

	return S_OK;
}

HRESULT CSoundManager::SetNextEnvironment(
	void )
{
	HRESULT hr;
	...
	if ( m_pEAXListener != NULL )
	{
		if ( FAILED( hr = m_pEAXListener->SetNextEnvironment()) )
			return DXTRACE_ERR( TEXT("m_pEAXListener->SetEnvironment"), hr );
...
//-----------------------------------------------------------------------------
HRESULT CSoundManager::InitializeListenerAttributes( void )
{
	LISTENERATTRIBUTES la;
	HRESULT hr = m_pEAXManager->GetListenerAttributes( &la );

	if ( hr != EM_OK )
		return DXTRACE_ERR( TEXT(" m_pEAXManager->GetListenerAttributes"), hr );

	m_pDSListener->SetDistanceFactor(la.fDistanceFactor,DS3D_DEFERRED);
	m_pDSListener->SetRolloffFactor(la.fRolloffFactor,DS3D_DEFERRED);
	m_pDSListener->SetDopplerFactor(la.fDopplerFactor,DS3D_DEFERRED);
	m_pDSListener->CommitDeferredSettings();
