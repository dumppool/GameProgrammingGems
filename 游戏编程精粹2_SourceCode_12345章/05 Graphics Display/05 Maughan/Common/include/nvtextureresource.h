...
public:
	...
	{
		NVTextureResource* pTexture = static_cast<NVTextureResource*>(NVRESOURCEMANAGER.FindResource(pszResource));
		if (pTexture)
		{
	...
	static LPDIRECT3DBASETEXTURE8 GetTexture(NVResourceID TextureID)
	{
		NVTextureResource* pTexture = static_cast<NVTextureResource*>(NVRESOURCEMANAGER.FindResource(TextureID));
		if (pTexture)
		{
			return pTexture->m_pTexture;
		}
...
{
public:
	NVTextureNormalMapFromResource(LPDIRECT3DDEVICE8 pD3DDev, NVResourceID BaseResourceID, D3DFORMAT Format = (D3DFORMAT)MAKEFOURCC('N', 'V', 'H', 'S'), D3DPOOL Pool = D3DPOOL_DEFAULT)
		: m_pD3DDev(pD3DDev),
		m_BaseResourceID(BaseResourceID),
		m_Format(Format),
		m_Pool(Pool)
	{}
	...
	virtual bool Invoke()
	{
		m_pTexture = NVTexture2::CreateNormalMap(m_pD3DDev, (LPDIRECT3DTEXTURE8)NVTextureResource::GetTexture(m_BaseResourceID), m_Format, m_Pool);
		if (m_pTexture)
			return true;
